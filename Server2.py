...
class obj(object):
    ...
    def retrieve(self,movieID,timestamp):
        server = 1
        if timestamp[1]<=value_timestamp[1]:#check value has all previous updates from client
            if timestamp[0]<=value_timestamp[0]:
                if timestamp[2]<=value_timestamp[2]:
                    found = False
    ...
    def update(self,opID,movieID,rating,timestamp):
        ...
        for i in range(len(update_log)):
            if update_log[i][-1] == opID:
                return True,timestamp
        replica_timestamp[server]+=1
        ts = [timestamp[0],replica_timestamp[1],timestamp[2]]
...
ns.register("example.server2", uri)
