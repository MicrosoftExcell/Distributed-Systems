...
class obj(object):
    ...
    def retrieve(self,movieID,timestamp):
        server = 0 
        if timestamp[0]<=value_timestamp[0]: #check value has all previous updates from client 
    ...
    def update(self,opID,movieID,rating,timestamp):
        ...
        for i in range(len(update_log)):
            if update_log[i][-1] == opID:
                return True,timestamp
        replica_timestamp[server]+=1
        ts = [replica_timestamp[0],timestamp[1],timestamp[2]]
...
ns.register("example.server1", uri)
